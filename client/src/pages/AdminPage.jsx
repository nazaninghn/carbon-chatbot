...
export default function AdminPage({ auth, lang, onBack }) {
  ...
  const deleteSession = async (id) => {
    ...
    await fetch(`/api/admin/sessions/${id}`, { method: 'DELETE', headers });
    loadData();
  };
    ...
    <div className="min-h-screen" style={{ background: '#f2f0ec' }}>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex gap-1 p-1 rounded-xl mb-8 w-fit" style={{ background: 'rgba(163,177,138,0.15)' }}>
          {[
            { id: 'dashboard', label: lang === 'tr' ? '📊 Dashboard' : '📊 Dashboard' },
        ...
        {tab === 'dashboard' && stats && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={i} className="rounded-2xl p-5" style={{ background: 'rgba(255,255,255,0.85)', border: '1px solid rgba(163,177,138,0.2)', backdropFilter: 'blur(8px)' }}>
        ...
        {/* Users */}
        {tab === 'users' && (
          <div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.85)', border: '1px solid rgba(163,177,138,0.2)' }}>
            <div className="px-5 py-3 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(163,177,138,0.15)' }}>
              <span className="text-sm font-semibold" style={{ color: C.pine }}>{users.length} {lang === 'tr' ? 'kullanıcı' : 'users'}</span>
              <button onClick={loadData} className="text-xs px-3 py-1 rounded-full" style={{ background: 'rgba(163,177,138,0.15)', color: C.fern }}>
                ↻ {lang === 'tr' ? 'Yenile' : 'Refresh'}
              </button>
            </div>
            <div className="divide-y" style={{ borderColor: 'rgba(163,177,138,0.1)' }}>
              {users.map(u => (
                <div key={u._id} className="px-5 py-3 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                  <div>
              ...
              {users.length === 0 && <p className="px-5 py-8 text-center text-sm" style={{ color: C.sage }}>No users yet</p>}
            </div>
          </div>
        )}
        ...
        {tab === 'sessions' && (
          <div className="rounded-2xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.85)', border: '1px solid rgba(163,177,138,0.2)' }}>
            <div className="px-5 py-3 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(163,177,138,0.15)' }}>
              <span className="text-sm font-semibold" style={{ color: C.pine }}>{sessions.length} {lang === 'tr' ? 'oturum' : 'sessions'}</span>
              <button onClick={loadData} className="text-xs px-3 py-1 rounded-full" style={{ background: 'rgba(163,177,138,0.15)', color: C.fern }}>
                ↻ {lang === 'tr' ? 'Yenile' : 'Refresh'}
              </button>
            </div>
            <div className="divide-y" style={{ borderColor: 'rgba(163,177,138,0.1)' }}>
              {sessions.map(s => (
                <div key={s._id} className="px-5 py-3 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                  <div>
                    <p className="text-sm font-medium" style={{ color: C.pine }}>{s.title || 'Untitled'}</p>
                    <p className="text-xs" style={{ color: C.fern }}>
                      {lang === 'tr' ? 'Aşama' : 'Phase'} {s.currentPhase} · {s.messages?.length || 0} {lang === 'tr' ? 'mesaj' : 'messages'}
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
              ...
              {sessions.length === 0 && <p className="px-5 py-8 text-center text-sm" style={{ color: C.sage }}>No sessions yet</p>}
            </div>
          </div>
        )}
      </div>
